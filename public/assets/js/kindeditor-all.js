...
KindEditor.plugin('image', function(K) {
	...
	self.plugin.imageDialog = function(options) {
		...
		var dialog = self.createDialog({
			...
			width : dialogWidth,
			height : dialogHeight,
			title : "上传文件：<span style='color:red;'>（请注意！文件大小不能超过2M）</span>",
			body : html,
			yesBtn : {
				name : self.lang('yes'),
				click : function(e) {
...
KindEditor.plugin('insertfile', function(K) {
	...
	self.plugin.fileDialog = function(options) {
		...
		var dialog = self.createDialog({
			name : name,
			width : 450,
			title : "插入文件：<span style='color:red;'>（请注意！文件大小不能超过2M）</span>",
			body : html,
			yesBtn : {
				name : self.lang('yes'),
				click : function(e) {
	...
	self.clickToolbar(name, function() {
		self.plugin.fileDialog({
			clickFn : function(url, title) {
				...
				var html = '{(<span style="color:red;">附件：</span><a class="ke-insertfile [[[{' + title + '}]]] {[[[' + url + ']]]}" data-ke-src="' + url + '" target="_blank">' + title + '</a>)}';
				self.insertHtml(html).hideDialog().focus();
			}
		});
	});
